...
books = pd.DataFrame({
    ...
    "Genre": [
         ...
         "Realistic Fiction", "Realistic Fiction", "Historical Fiction", "Historical Fiction", "Realistic Fiction",
...
books.loc[books["Title"] == "Franny K. Stein: Mad Scientist (Series) - Jim Benton", "Genre"] = "Science Fiction-Humor"
...
genre_options = ["No preference", "Fantasy", "Animal Fiction", "Mystery", "Adventure", "Science Fiction", "Historical Fiction", "Realistic Fiction", "Non-Fiction", "Magical Realism (blend of fantasy and reality)"]
    ...
    if selected_genre.lower() != "no preference":
        ...
        for i, genre in enumerate(filtered_books["Genre"]):
            ...
            if selected_genre_lower not in book_genres:
                genre_penalty[i] = 12  # Fixed penalty for genre mismatch

    # Total distance includes genre penalty
    total_distance = base_distance + genre_penalty

    # Convert to similarity percentage
    max_distance = np.sqrt(461.7) + 12  # Adjusted max with possible genre penalty
